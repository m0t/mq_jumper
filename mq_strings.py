...
class mq_initial_data:
    ...
    def rawData(self):
        ...
        data = data+self._initial_data["channel_name"][0].encode()
        ...
        data = data+self._initial_data["queue_manager"][0].encode()+pack('>LH', self._initial_data["heartbeat"][0], self._initial_data["unknown4"][0])
        return data
...
class static_data:
    ...
    def print_pcf_int(self, field_type, field_data):
        if field_type in self._pcf_int_codes:
    ...
    def print_reason(self, reason_string,verbose):
        if reason_string in self._reason_codes:
            if verbose == 1:
    ...
    def print_completion(self, completion_string,verbose):
        if completion_string in self._completion_codes:
            if verbose == 1:
    ...
    def print_status(self, status_string, verbose):
        if status_string in self._status_codes:
            if verbose == 1:
